...
public final class AccessibilityHelper {
     ...
     * 递归通过组件名字查找
     */
    public static AccessibilityNodeInfo findNodeInfosByClassNameRecursion(AccessibilityNodeInfo nodeInfo, String className) {
        AccessibilityNodeInfo rootNode = findNodeInfosByClassName(nodeInfo, className);
        if (rootNode == null && !TextUtils.isEmpty(className) && nodeInfo != null) {
            for (int i = 0; i < nodeInfo.getChildCount(); i++) {
                AccessibilityNodeInfo childNode = findNodeInfosByClassNameRecursion(nodeInfo.getChild(i), className);
                if (childNode != null) {
                    return childNode;
                }
            }
